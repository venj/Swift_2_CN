...
本导览将通过向你展示如何实现各种编程任务，为你开始编写Swift代码提供足够的信息。如果你遇到了一些你不理解的知识点也不用担心－－所有在本章中介绍的知识点，在后文中都将一一详述。
*/
...
> **说明**
>
> 在Mac中，下载下面的Playground文件，双击在Xcode中打开它：[https://developer.apple.com/go/?id=prerelease-swift-tour]( https://developer.apple.com/go/?id=prerelease-swift-tour )
*/
...
一个常量或变量必须与其赋的值有相同的类型。不过你不需要总是显式地写出类型。你可以在创建常量和变量的时候直接赋值，让编译器来推定（infer）其类型。在上面的例子中，编译器会把`myVariable`推定为整数型，因为它的初始值是一个整数。
